...
public class ModularUnexpected implements BehaviourGenerator {
  ...
  public ModularUnexpected(List<DefaultProblem> problems, int radius, int goalID,
  ...
  public Action generateAction(State state, Logger logger) throws NoValidActionException {
    ...
    switch (currentStage) {
      case CurrentStage.APPROACHING:
        ...
        if (generators.get(CurrentStage.APPROACHING).distanceToGoal(state) <= radius) {
          logger.logSimple("Reached Radius. Switching to unexpected!");
          currentStage = CurrentStage.UNEXPECTED;
        }

        break;

      case CurrentStage.UNEXPECTED:
        logger.logSimple("Current stage: Unexpected");
        break;
    }
